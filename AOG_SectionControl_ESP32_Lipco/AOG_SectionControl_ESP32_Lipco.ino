...
byte vers_nr = 47;
char VersionTXT[150] = " - 15. Mai 2022 by MTZ8302<br>(Lipco Edition)";
...
struct set {
	...
	uint8_t LEDWiFi_PIN = 2;// 5;		// WiFi Status LED 255 = off
	...
	char password4[24] = "aveasillrac";                // WiFi network password//Accesspoint name and password
	char ssid5[24] = "";          // WiFi network Client name
	char password5[24] = "";                // WiFi network password//Accesspoint name and password

	char ssid_ap[24] = "LipcoSectionControlNet"; // name of Access point, if no WiFi found, no password!!
...
//Lipco: left=Fronttank  right=rear=hinten	
	uint8_t RateControlAutoManSW_PIN = 102;	// not supported at the moment
	...
	uint8_t RateSWLeft_PIN = 36;			// Rate +/- switch (ON)-0-(ON) toggle switch (GND)- 1.75V - (3,3V)
	uint8_t RateControlPWM = 0;				// 1 if PWM valve, 0 if Motor drive for pressure change		

	uint8_t	FlowDirLeft_PIN = 128;			// Rate-Control Valve/Motor Direktion
	uint8_t	FlowPWMLeft_PIN = 129;			// Rate-Control Valve PWM/Motor ON/OFF
	uint8_t	FlowEncALeft_PIN = 255;			// Flowmeter left/1

	uint8_t RateControlRightInst = 0;	    // 1 if Rate control is there, else: 0
	uint8_t RateSWRightInst = 1;			// 1 if Rate control Pressure switch is there, else: 0	
	uint8_t	RateSWRight_PIN = 35;			// Rate +/- switch
	uint8_t	FlowDirRight_PIN = 130;			// 255  = unused Rate-Control Valve Direktion
	uint8_t	FlowPWMRight_PIN = 131;			// 255  = unused Rate-Control Valve PWM
	uint8_t	FlowEncARight_PIN = 255;		// Flowmeter right/2 

	uint8_t SectNum = 2;//HL,HR,VL,VR,Agitator,Recycling,Fan,255,TunLNar,TunLWider,TunRNar,TunRWider				// number of sections
	...
	uint8_t Relay_PIN[16] = { 116,117,118,119,120,121,122,255,124,125,126,127,255,255,255,255 };  //GPIOs of ESP32 OUT to sections of sprayer HIGH/3.3V = ON
	...
	uint8_t SectSW_PIN[16] = { 100,101,255,255,255,255,255,255,255,255,255,255,255,255,255,255 };//section switches to GPIOs of ESP32 GND = section off, open/+3.3V section auto/on
	...
	uint8_t	SectMainSW_PIN = 39;			// ESP32 to AOG Main auto toggle switch open=nothing/AOG button GND=OFF +3,3=AOG Auto on	OR connected to hitch level sensor	
	uint8_t	SectAutoManSW_PIN = 34;			// Main Auto/Manual switch 39:!!no internal pullup!!

	uint8_t DocOnly = 0;					// 0: use as section control, 1: Documentation only = AOG writes the state of the input switches


	//Lipco mod: PINs
	uint8_t FronttankSW_PIN = 33;
//	uint8_t AgitatorSW_PIN = 103;
//	uint8_t AgitatorRel_PIN = 120;
//	uint8_t RecyclingSW_PIN = 104;
//	uint8_t RecyclingRel_PIN = 121;
//	uint8_t FanSW_PIN = 105;
//	uint8_t FanRel_PIN = 122;
	uint8_t TunnelLeftSW_PIN = 32;
//	uint8_t TunnelLeftNarrowerRel_PIN = 124;
//	uint8_t TunnelLeftWidererRel_PIN = 125;
//	uint8_t TunnelRightNarrowerSW_PIN = 14;
//	uint8_t TunnelRightWiderSW_PIN = 0;
//	uint8_t TunnelRightNarrowerRel_PIN = 126;
//	uint8_t TunnelRightWidererRel_PIN = 127;
...
#include <Wire.h>//needed for MCP
//#include "zAOG_MCP23017.h"


//MCP23017 mcpForInput = MCP23017(0x20);
//MCP23017 mcpForOutput = MCP23017(0x21);
...
bool task_WiFiConnectRunning = false;
...
TaskHandle_t taskHandle_WiFiConnectionCheckLEDBlink;
...
int FronttankSWVal = (SWOFF + SWON) / 2;
int TunnelLeftSWVal = (SWOFF + SWON) / 2;
...
boolean RateAuto = false; //not supported at the moment
...
void setup()
{
	...
	//set GPIOs
	assignGPIOs();  //also starts wire/MCP if needed
	...
	xTaskCreate(WiFi_LED_blink_and_Connection_check, "WiFiLEDBlinkAndConnectionCheck", 3072, NULL, 0, &taskHandle_WiFiConnectionCheckLEDBlink);
	delay(500);
